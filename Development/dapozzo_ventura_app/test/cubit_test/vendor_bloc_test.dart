...
class MockGoodTypologyRepository extends Mock implements GoodsTypologyRepository {}
...
void mainCubit() {
  group('MarketPlace BLoc Test', () {
    ...
    setUp(() {
      ...
      when(repo.getGoodsTypologies([], -1, 1))
    ...
    test(
        'test that repository mock work as intended returning a list of goods',
            () async {
          expect(await repo.getGoodsTypologies([], -1, 1), isA<List<GoodTypologyModel>>());
        });
    ...
    blocTest("test that calling restore from a searched state the output is a initialState",
        build: () {final cubit= VendorCubit(repo);
